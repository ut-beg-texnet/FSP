...
function input_distribution_histograms_to_d3(
    ...
    # Define display names for visualization
    display_names = Dict(
        "vertical_stress" => "S_{Vertical}",
        "min_horizontal_stress" => "S_{hmin}",
        "max_horizontal_stress" => "S_{Hmax}",
        "pore_pressure" => "Natural Pore Pressure",
        "max_stress_azimuth" => "S_{Hmax} Azimuth",
        "aphi_value" => "aPhi",
    ...
    # Process each fault
    for fault_id in fault_ids
        fault_idx = parse(Int, fault_id)
        
        # Extract the data for this fault
        fault_data = mc_results[mc_results.FaultID .== fault_id, :]
        
        # Process stress parameters - these are the same for all faults
        for param in stress_params
            if haskey(stress_param_values, param)
                ...
                uncertainty_param = param == "vertical_stress" ? "vertical_stress_gradient_uncertainty" :
                                   ...
                                   param == "max_horizontal_stress" ? "max_horizontal_stress_uncertainty" :
                                   param == "pore_pressure" ? "initial_pore_pressure_gradient_uncertainty" :
                                   param == "max_stress_azimuth" ? "max_stress_azimuth_uncertainty" :
                                   param == "aphi_value" ? "aphi_value_uncertainty" : ""
                
                # Get the base value and uncertainty
                base_value = stress_inputs[param]
                uncertainty = haskey(uncertainties, uncertainty_param) ? 
                            (isa(uncertainties[uncertainty_param], Number) ? uncertainties[uncertainty_param] : 
        ...
        # Process fault-specific parameters
        for param in ["Strike", "Dip", "FrictionCoefficient"]
            if haskey(fault_param_values[fault_idx], param)
                ...
                # Get uncertainty parameter name
                uncertainty_param = param == "Strike" ? "strike_angles_uncertainty" :
                                   param == "Dip" ? "dip_angles_uncertainty" :
                                   param == "FrictionCoefficient" ? "friction_coefficient_uncertainty" : ""
        ...
        # Add the slip pressure distribution for this fault
