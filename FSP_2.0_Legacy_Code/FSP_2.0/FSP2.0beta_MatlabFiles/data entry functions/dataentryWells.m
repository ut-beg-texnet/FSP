...
radioButtonGroupWells=uibuttongroup('Position', [0 .85 .8 .18],'backgroundcolor',[1 1 1],...
    'SelectionChangeFcn', {@dropDownMenuWells},'parent',loadWellsFigure,'fontsize',hDV.ftsz,...
    'BorderType','none','fontunits','normalized');

radioButton1=uicontrol(radioButtonGroupWells,'Style','radiobutton',...
    'Position', [0.01 .45 .98 .5],'backgroundcolor',[1 1 1],...
    'String','Enter Wells Manually','fontunits','normalized',...
    'ToolTipString','Select to enter constant inhjection rate wells');

radioButton2=uicontrol(radioButtonGroupWells,'Style','radiobutton',...
    'Position', [0.01 .0 .98 .5],'backgroundcolor',[1 1 1],...
    'String','Load Wells Complete .csv','fontunits','normalized',...
...
if isfield(hDV.data.realWellData,'use')
    ...
    if hDV.data.realWellData.use==1
        set(radioButtonGroupWells,'SelectedObject',radioButton2);
    end
end
...
%this must tie back to callbackdatabuts.m
    function butcall(~,~,hDV,htxt,~)
        ...
        %         switch val
        resetRedFlag=0;
        if get(radioButtonGroupWells,'selectedobject')==radioButton1
            %             case 1 % constant wells rate
            for p=1:P
                for q=1:Q
                    vec(p,q)=str2double(get(htxt(p,q),'string'));
                        if (hDV.data.inject.vals(p,q)~=vec(p,q)) && resetRedFlag==0 % if values changed, choose to reset calculate buttons red
                        resetRedFlag=1;
                        end
                end
            end
...
%             hDV.data.inject.vals=zeros(P,Q); %this needs to be set somehow
        ...
        if resetRedFlag % turn all calculate buttons red, except prob geomechanics
