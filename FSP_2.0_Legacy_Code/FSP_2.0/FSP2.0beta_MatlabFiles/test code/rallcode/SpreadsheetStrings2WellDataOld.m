...
hDV.data.realWellData.XEasting=str2double(hDV.data.realWellData.stringsWellDataAdvanced(IA,2));% well type stored once per well
hDV.data.realWellData.YNorthing=str2double(hDV.data.realWellData.stringsWellDataAdvanced(IA,3));% well type stored once per well
...
for CycleWellsK=1:hDV.data.nwells % cycle over wells
    ...
    % these wil be hSV.data.realWellData. ...
...
%     hDV.data.realWellData.YNorthing(CycleWellsK,1)=C(CycleWellsK,3);
    ...
    for CycleMonthsK=1:size(YearMonthVolumePressureThisWell,1)
        ...
        decyearBarrelsPerDayThisWell(end+1,[1:2])=[startDateThisMonthdecyear,barrelsPerDayOverThisMonth]; % make entry at beginning of month
        decyearBarrelsPerDayThisWell(end+1,[1:2])=[endDateThisMonthdecyear,barrelsPerDayOverThisMonth]; % make entry at end of month
        
        datenumBarrelsPerDayThisWell(end+1,[1:2])=[startDateThisMonthdatenum,barrelsPerDayOverThisMonth]; % make entry at beginning of month
        ...
        if any(YearMonthVolumePressureThisWell(:,1)==nextYear & YearMonthVolumePressureThisWell(:,2)==nextMonth) % if there is a data entry for next month
            ...
            decyearBarrelsPerDayThisWell(end+1,[1:2])=[startDateNextMonthdecyear,0]; % make entry of 0 barrels next month
            ...
            datenumBarrelsPerDayThisWell(end+1,[1:2])=[startDateNextMonthdatenum,0]; % make entry at beginning of month
            end
    ...
    assumeInjectionExtendsToEnd=1;
    
    if assumeInjectionExtendsToEnd
